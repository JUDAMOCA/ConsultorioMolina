...
// Tipos
// ─────────────────────────────────────────────
interface Service { id: string; name: string; duration_minutes: number; price_cop: number }
...
export default function AgendarPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!u) { router.push('/auth/login?redirect=/agendar'); return }
      setUser(u)

      // Check role (don't let dentist book)
      const { data: profile } = await supabase.from('profiles').select('role').eq('id', u.id).single()
      if (profile?.role === 'dentist') { router.push('/panel'); return }

      // Load services and clinic info in parallel
  ...
  useEffect(() => {
    if (!selectedDate || !selectedService || !clinicInfo) return
    const loadSlots = async () => {
      ...
      const morningSlots = generateSlots(clinicInfo.morning_start, clinicInfo.morning_end, duration, bookedSlots)
      const afternoonSlots = generateSlots(clinicInfo.afternoon_start, clinicInfo.afternoon_end, duration, bookedSlots)
  ...
  const handleConfirm = async () => {
    if (!selectedService || !selectedDate || !selectedTime || !paymentMethod || !user) return
    setError('')
    setSaving(true)
  ...
  // Render
  // ─────────────────────────────────────────────
  const availableDates = clinicInfo ? getAvailableDates(clinicInfo.working_days ?? [1,2,3,4,5]) : []

  const isMorningSlot = (t: string) => {
    if (!clinicInfo) return false
    return timeToMin(t) < timeToMin(clinicInfo.afternoon_start)
  }
  ...
  return (
    <main className="min-h-screen bg-sky-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        {dataLoading ? (
          <div className="card text-center py-16 text-slate-400">Cargando...</div>
        ) : (
          <>
            {/* ── STEP 1: Servicio ── */}
            {step === 'service' && (
            ...
            {step === 'wompi' && selectedService && appointmentId && (
              <div>
                ...
                <WompiButton
                  amountCOP={selectedService.price_cop}
                  reference={appointmentId}
                  redirectUrl={`${process.env.NEXT_PUBLIC_SITE_URL || ''}/pagos/resultado`}
                />
