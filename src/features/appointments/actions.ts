...
export async function cancelAppointment(id: string): Promise<CancelResult> {
  ...
  if (!appt || appt.patient_id !== user.id) return { error: 'Cita no encontrada.' }
  if (appt.status === 'cancelled') return { error: 'La cita ya está cancelada.' }
  if (!canCancel(appt)) return { error: 'Solo puedes cancelar con más de 1 hora de anticipación.' }
