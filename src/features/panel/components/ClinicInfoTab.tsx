...
export default function ClinicInfoTab({
  ...
  if (!info) {
    return (
      <div className="card max-w-2xl py-12 text-center text-slate-400">
  ...
  const toggleDay = (d: number) => {
    ...
    const working_days = current.includes(d)
    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-slate-500 text-sm">
          Esta información se muestra en la página de inicio y afecta los
          horarios de citas
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="card">
          ...
          <div className="mb-5 flex gap-2">
            {(
              ...
              <button
                ...
                className={`rounded-xl border-2 px-3 py-2 font-medium text-sm transition-colors ${info.banner_type === key ? "border-sky-600 bg-sky-600 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-sky-300"}`}
              >
                {label}
              </button>
            ))}
          ...
          {/* Preview */}
          <div
            className="relative mb-4 h-24 w-full overflow-hidden rounded-xl"
            style={
          ...
          >
            ...
            {info.banner_type === "slider" &&
              (info.banner_images ?? []).length === 0 && (
                <div className="flex h-full w-full items-center justify-center bg-slate-200 text-slate-400 text-sm">
                  Sin fotos aún
                </div>
              )}
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="font-bold text-sm text-white opacity-60">
                Vista previa
              </span>
            </div>
          </div>
          ...
          {(info.banner_type === "gradient" ||
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="label" htmlFor="banner-color-from">
                  {info.banner_type === "gradient"
                    ? "Color inicial"
                    : "Color de fondo"}
                </label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setInfo({ ...info, banner_color_from: e.target.value })
                    }
                  />
                  <input
              ...
              {info.banner_type === "gradient" && (
                <div className="flex-1">
                  <label className="label" htmlFor="banner-color-to">
                    Color final
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      id="banner-color-to"
                      type="color"
                      aria-label="Selector de color final"
                      className="h-10 w-10 cursor-pointer rounded-lg border border-slate-200"
                      value={info.banner_color_to ?? "#06b6d4"}
                      onChange={(e) =>
                        setInfo({ ...info, banner_color_to: e.target.value })
                      }
                    />
                    <input
                      className="input flex-1"
                      aria-label="Color final en hexadecimal"
                      value={info.banner_color_to ?? "#06b6d4"}
                      onChange={(e) =>
                        setInfo({ ...info, banner_color_to: e.target.value })
                      }
          ...
          {info.banner_type === "slider" && (
            <div>
              <input
                ...
                accept="image/*"
                multiple
                aria-label="Agregar fotos al banner"
              ...
              <p className="mb-3 text-slate-400 text-xs">
                Puedes subir varias fotos. Pasarán automáticamente cada 4
                segundos.
              </p>
              {(info.banner_images ?? []).length > 0 && (
                  ...
                  {info.banner_images.map((url, i) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 rounded bg-red-500 px-1.5 py-0.5 text-white text-xs opacity-0 transition-opacity group-hover:opacity-100"
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setInfo({ ...info, clinic_name: e.target.value })
                }
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setInfo({ ...info, banner_subtitle: e.target.value })
                }
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {info.logo_url && (
                    <button
                      type="button"
                      onClick={() => setInfo({ ...info, logo_url: null })}
        ...
        <div className="card">
          <h3 className="mb-4 font-bold text-slate-700">
            ✨ Textos de características (página de inicio)
          </h3>
          <div className="space-y-5">
            {(
              ...
              <div key={n} className="space-y-2 rounded-xl bg-slate-50 p-4">
                <p className="font-medium text-slate-500 text-xs">
                  {icon} Característica {n}
                </p>
                <input
                  className="input"
                  placeholder="Título"
                  aria-label={`Título característica ${n}`}
                  value={(info[featureKey(n, "title")] as string) ?? ""}
                  onChange={(e) =>
                    setInfo({
                      ...info,
                      [featureKey(n, "title")]: e.target.value,
                    } as Partial<PanelClinicInfo> as PanelClinicInfo)
                  }
                />
                <textarea
                  className="input h-16 resize-none"
                  placeholder="Descripción"
                  aria-label={`Descripción característica ${n}`}
                  value={(info[featureKey(n, "desc")] as string) ?? ""}
                  onChange={(e) =>
                    setInfo({
                      ...info,
                      [featureKey(n, "desc")]: e.target.value,
                    } as Partial<PanelClinicInfo> as PanelClinicInfo)
                  }
                />
              </div>
        ...
        {/* Días laborales */}
        <div className="card">
          <h3 className="mb-4 font-bold text-slate-700">📅 Días de atención</h3>
          <div className="flex flex-wrap gap-2">
            {[0, 1, 2, 3, 4, 5, 6].map((d) => (
              <button
                ...
                type="button"
                onClick={() => toggleDay(d)}
                className={`rounded-xl border-2 px-4 py-2 font-medium text-sm transition-colors ${(info.working_days ?? [1, 2, 3, 4, 5]).includes(d) ? "border-sky-600 bg-sky-600 text-white" : "border-slate-200 bg-white text-slate-600 hover:border-sky-300"}`}
              >
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            <div>
              <p className="mb-2 font-medium text-slate-600 text-sm">
                Jornada mañana
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label" htmlFor="morning-start">
                    Inicio
                  </label>
                  <input
                    id="morning-start"
                    type="time"
                    className="input"
                    value={info.morning_start}
                    onChange={(e) =>
                      setInfo({ ...info, morning_start: e.target.value })
                    }
                  />
                </div>
                <div>
                  <label className="label" htmlFor="morning-end">
                    Fin
                  </label>
                  <input
                    id="morning-end"
                    type="time"
                    className="input"
                    value={info.morning_end}
                    onChange={(e) =>
                      setInfo({ ...info, morning_end: e.target.value })
                    }
                  />
                </div>
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label" htmlFor="afternoon-start">
                    Inicio
                  </label>
                  <input
                    id="afternoon-start"
                    type="time"
                    className="input"
                    value={info.afternoon_start}
                    onChange={(e) =>
                      setInfo({ ...info, afternoon_start: e.target.value })
                    }
                  />
                </div>
                <div>
                  <label className="label" htmlFor="afternoon-end">
                    Fin
                  </label>
                  <input
                    id="afternoon-end"
                    type="time"
                    className="input"
                    value={info.afternoon_end}
                    onChange={(e) =>
                      setInfo({ ...info, afternoon_end: e.target.value })
                    }
                  />
                </div>
              </div>
          ...
          <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sky-700 text-sm">
            💡 Los cambios de horario se reflejan automáticamente al agendar
            nuevas citas.
          </div>
        </div>
