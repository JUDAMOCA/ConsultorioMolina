...
import { generateSlots, hasConflict, minToTime, timeToMin, type BookedSlot } from './lib/slots'
...
async function bookedSlotsForDate(dateKey: string): Promise<BookedSlot[]> {
  ...
  return (data ?? []).map((a) => ({ s: timeToMin(a.start_time), e: timeToMin(a.end_time) }))
}
...
export async function getAvailableSlots(serviceId: string, dateKey: string): Promise<string[]> {
  ...
  const [clinic, booked] = await Promise.all([getClinicInfo(), bookedSlotsForDate(dateKey)])

  return [
    ...generateSlots(clinic?.morning_start ?? '08:00', clinic?.morning_end ?? '12:00', duration, booked),
    ...generateSlots(clinic?.afternoon_start ?? '14:00', clinic?.afternoon_end ?? '18:00', duration, booked),
  ]
}
...
export async function createAppointment(input: {
  ...
  if (error || !data) {
    return { error: 'No se pudo guardar la cita: ' + (error?.message ?? 'intenta de nuevo') }
  }
  return { id: data.id }
}
