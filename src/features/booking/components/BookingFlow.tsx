...
export default function BookingFlow({
  ...
  const handleConfirm = async () => {
    if (!state.service || !state.dateKey || !state.time || !state.paymentMethod)
      return;
  ...
  const availableDates = getAvailableDates(
    clinicInfo?.working_days ?? [1, 2, 3, 4, 5],
  );
  const selectedDate = state.dateKey
    ? new Date(`${state.dateKey}T12:00:00`)
    : null;
  const isMorningSlot = (t: string) =>
    timeToMin(t) < timeToMin(clinicInfo?.afternoon_start ?? "14:00");
  ...
  return (
    <>
        ...
        <div className="mb-8 flex items-center justify-center gap-2">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full font-bold text-sm transition-colors ${
                  state.step === s
                    ...
                    : STEPS.indexOf(state.step as (typeof STEPS)[number]) > i
              ...
              {i < 3 && <div className="h-0.5 w-8 bg-slate-200" />}
            </div>
          ))}
        </div>
      )}
      ...
      {state.step === "service" && (
        <div>
          <h2 className="mb-4 font-bold text-lg text-slate-700">
            ...
            <div className="space-y-3">
              {services.map((svc) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    dispatch({ type: "pickService", service: svc })
                  }
                  className={`card w-full border-2 text-left transition-all hover:border-sky-300 hover:shadow-md ${state.service?.id === svc.id ? "border-sky-500 bg-sky-50" : "border-transparent"}`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-slate-400 text-xs">COP</p>
                    </div>
                  </div>
                </button>
              ))}
      ...
      {state.step === "date" && (
        <div>
          <button
            ...
            className="mb-4 block text-sky-600 text-sm hover:underline"
          >
            ← Volver
          </button>
          <h2 className="mb-1 font-bold text-lg text-slate-700">
            2. Selecciona la fecha
          </h2>
          <p className="mb-4 text-slate-500 text-sm">
            Servicio: <strong>{state.service?.name}</strong>
          </p>
              ...
              {availableDates.map((d) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border-2 p-3 text-center transition-all ${isSelected ? "border-sky-500 bg-sky-50" : "border-slate-200 bg-white hover:border-sky-300"}`}
                  >
      ...
      {state.step === "time" && (
        <div>
          <button
            type="button"
            onClick={() => dispatch({ type: "goto", step: "date" })}
            className="mb-4 block text-sky-600 text-sm hover:underline"
          >
            ← Volver
          </button>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {morningSlots.map((t) => (
                      <button
                        key={t}
                        type="button"
                        onClick={() => dispatch({ type: "pickTime", time: t })}
                        className={`rounded-xl border-2 py-2 font-semibold text-sm transition-all ${state.time === t ? "border-sky-500 bg-sky-50 text-sky-700" : "border-slate-200 bg-white text-slate-700 hover:border-sky-300"}`}
                      >
                        {t}
                      </button>
                    ))}
      ...
      {state.step === "payment" && (
        <div>
          <button
            type="button"
            onClick={() => dispatch({ type: "goto", step: "time" })}
            className="mb-4 block text-sky-600 text-sm hover:underline"
          >
            ← Volver
          </button>
            ...
            <div className="space-y-2 text-slate-600 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  ...
                  {state.time
                    ? minToTime(
                        timeToMin(state.time) +
                          (state.service?.duration_minutes || 0),
                      )
                    : ""}
                </span>
              </div>
              <div className="flex justify-between border-slate-100 border-t pt-2 font-bold text-base text-slate-800">
                <span>Total</span>
                <span className="text-sky-600">
                  ${state.service?.price_cop.toLocaleString("es-CO")} COP
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6 space-y-3">
            <button
              type="button"
              onClick={() => dispatch({ type: "pickPayment", method: "cash" })}
              className={`card w-full border-2 text-left transition-all ${state.paymentMethod === "cash" ? "border-sky-500 bg-sky-50" : "border-transparent hover:border-sky-200"}`}
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Paga directamente en el consultorio
                  </p>
                </div>
                {state.paymentMethod === "cash" && (
                  <div className="ml-auto text-sky-600 text-xl">✓</div>
                )}
              </div>
            </button>
            <button
              type="button"
              onClick={() =>
                dispatch({ type: "pickPayment", method: "online" })
              }
              className={`card w-full border-2 text-left transition-all ${state.paymentMethod === "online" ? "border-sky-500 bg-sky-50" : "border-transparent hover:border-sky-200"}`}
            >
              <div className="flex items-center gap-4">
                <div className="text-3xl">💳</div>
                <div>
                  ...
                  <p className="text-slate-500 text-sm">
                    Tarjeta de crédito/débito o PSE via Wompi
                  </p>
                </div>
          ...
          <button
            ...
            disabled={!state.paymentMethod || state.saving}
      ...
      {state.step === "wompi" && state.service && state.appointmentId && (
        <div>
          <div className="card mb-6 text-center">
            ...
            <h2 className="mb-2 font-bold text-slate-800 text-xl">
              Pago en línea
            </h2>
            <p className="text-slate-500 text-sm">
              Tu cita está guardada. Completa el pago para confirmarla.
            </p>
            <div className="mt-4 rounded-xl bg-slate-50 p-4 text-slate-600 text-sm">
              <p>
                <strong>{state.service.name}</strong>
              </p>
              <p className="mt-1 font-bold text-sky-600 text-xl">
                ${state.service.price_cop.toLocaleString("es-CO")} COP
              </p>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={() => dispatch({ type: "goto", step: "success" })}
            className="mt-4 w-full py-2 text-slate-400 text-sm hover:text-slate-600"
          >
      ...
      {state.step === "success" && (
        <div className="card py-10 text-center">
          ...
          <h2 className="mb-2 font-bold text-2xl text-slate-800">
            ¡Cita agendada!
          </h2>
          <p className="mb-6 text-slate-500">
            Tu cita ha sido registrada exitosamente.
          </p>
          <div className="mb-6 space-y-2 rounded-2xl bg-sky-50 p-5 text-left text-sm">
            <div className="flex justify-between">
