...
export function hasConflict(start: number, end: number, bookedSlots: BookedSlot[]): boolean {
  return bookedSlots.some((b) => start < b.e && end > b.s)
}

/** Upcoming dates (excluding today) that fall on a working day. */
export function getAvailableDates(workingDays: number[] = [1, 2, 3, 4, 5], weeksAhead = 4): Date[] {
  const dates: Date[] = []
