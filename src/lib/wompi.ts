...
import { sendAppointmentPaidEmail } from "@/lib/emails";
...
export async function reconcileAppointmentPayment(
  ...
  const { data: appointment, error } = await admin
    .from("appointments")
    .select(
      "id, payment_status, status, payment_method, appointment_date, start_time, services(name, price_cop), profiles(full_name, email, phone)",
    )
    .eq("id", appointmentId)
    .maybeSingle();
  ...
  if (input.status !== "APPROVED")
    return { outcome: "declined", appointmentId };
  ...
  const priceCop = Number(
    (service as { name?: string; price_cop?: number } | null)?.price_cop ?? 0,
  );
  ...
  // Use .select('id').maybeSingle() to detect whether this is the first confirmation
  // (row wasn't already paid). If no row is returned, the update was a no-op
  // (duplicate call) and we skip the email.
  ...
  if (updatedRow) {
    ...
    const svc = service as { name?: string; price_cop?: number } | null;
    await sendAppointmentPaidEmail({
      patientName:
        (profile as { full_name?: string } | null)?.full_name ?? "Paciente",
      patientEmail: (profile as { email?: string } | null)?.email ?? "",
      patientPhone:
        (profile as { phone?: string | null } | null)?.phone ?? null,
      serviceName: svc?.name ?? "Servicio",
      amountPaidCop: (input.amountInCents ?? 0) / 100,
      appointmentDate:
        (appointment as { appointment_date?: string }).appointment_date ?? "",
      startTime: (appointment as { start_time?: string }).start_time ?? "",
      appointmentId,
    });
  }
 ...
 */
export async function confirmAppointmentByTransaction(
  ...
  const [error, transaction] =
    await getWompiClient().transactions.getTransaction(transactionId);
  if (error || !transaction)
    return { outcome: "error", appointmentId: null, transaction: null };
